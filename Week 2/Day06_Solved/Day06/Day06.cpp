...
void printGrades(const std::map<std::string, double>& courseGrades)
{
    std::cout << "----GRADES----\n";
    for (const auto& [name, grade] : courseGrades)
    {
        std::cout << std::setw(10) << std::left << name << std::setw(4) << std::right << grade << "\n";
    }
}
...
int main()
{
    ...
    */
    ...
    std::cout << "Please enter the student to drop: ";
    std::string student;
    std::getline(std::cin, student);

    int numberFound = grades.erase(student);
    if (numberFound > 0)
    {
        std::cout << student << " was removed from the course.\n";
        printGrades(grades);
    }
    else
        std::cout << student << " was not found in the course.\n";
}