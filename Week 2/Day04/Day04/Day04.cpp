...
{
	//some condition that would 
	//for(N=0;N<300;N++)
	if (N >= 300) return; //an exit condition

	std::cout << N << "\n";
	recursiveLoop(N + 1);//this causes the loop
	std::cout << N << "\n";
...
{
	int size = unsorted.size();//n:=length(A)
	bool swapped = false;
	do //repeat
	{
		swapped = false;
		...
		--size;
	} while (swapped); //while we swapped something
}//end procedure
...
void Bats(int k = 0)
{
	if (k < 100)
	{
		...
		Bats(++k);
		//k++ uses the k value then increments k
		//++k increments k then uses its value
...
int main()
{
	std::vector<int> nummies{ 420, 0, 13, 5, 7 };
	std::cout << "\nUNSORTED\n";
